function [pseudoranges_minimized,satellite_positions_minimized] = minimize_PDOP(satellite_positions,pseudoranges,position_estimate,n_input)
%MINIMIZE_PDOP Summary of this function goes here
...
n=n_input;%Number of satellites to use as a sub-constellation
    ...
    for j=1:size(combs_DOP,1)
        ...
        %Get the pseudoranges of the satellite subset
        pseudoranges_aux=pseudoranges(combs_DOP(j,:));
        ...
        PDOP_current = sqrt(trace(H(1:3,1:3)));
...
pseudoranges_minimized=pseudoranges(combs_DOP(min_PDOP_index,:));
